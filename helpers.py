...
time = eventlet.import__patched('time')
import redis
...
def format_url(url):
	# make sure URLs aren't relative, and strip away unnecessary queries
	u = urlpase(url)
...
def log(msg):
	if settings.log_stdout:
		try:
			...
			pass # squah non-ascii logging errors


def get_proxy():
	# choose a proxy server to use for this request if needed
...
def enqueue_url(u):
	url = format_url(u)
	return redis.sadd("listening_url_queue", url)

def dequeue_url():
	return redis.spop("listening_url_queue")


if __name__ == '__main__':
	# Test proxy IP masking
